...
// --- VERIFICACIÓN DE EXISTENCIA DE GRÁFICOS ---
// Solo inicializamos si existe el elemento en el HTML actual (para evitar errores en modelo.html/costos.html)
...
if(donutCanvas) {
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => { if (entry.isIntersecting) animateCounterTo(computeTotal(), 2500); });
    }, { threshold: 0.5 });
    observer.observe(donutCanvas);
}

// --- GENERACIÓN DE TARJETAS DE ESTADÍSTICAS ---
...
function buildYearCard(){
  ...
  for(let i=0;i<labels.length;i++){
    ...
    row.innerHTML = `<div class="year-badge">${labels[i]}</div><div class="data-group"><div class="data-point" title="Incendios"><i class="fa-solid fa-fire"></i> ${formatNumber(incendios[i])}</div><div class="data-point" title="Superficie (ha)"><i class="fa-solid fa-tree"></i> ${formatNumber(superficie[i])} ha</div></div>`;
    el.appendChild(row);
  }
}
...
function buildGeneralCard(){
  ...
  h += createMetric('Mediana Incendios', formatNumber(medianInc), 'Valor central', 'accent-orange'); // RESTAURADA
  h += createMetric('Pico Máximo', formatNumber(maxInc), `Año ${maxIncYear}`, 'accent-red'); // RESTAURADA
  h += createMetric('Tendencia', `${pctChange > 0 ? '+' : ''}${pctChange.toFixed(1)}%`, 'Cambio Total', pctChange > 0 ? 'accent-red' : 'accent-green');
  
  grid.innerHTML = h;
}

// Ejecutar funciones si existen los elementos
buildYearCard(); 
buildGeneralCard();
...
(function(){
  ...
  if(frame) { // Solo ejecutar si existe el iframe (en modelo.html)
      function loadUrl(raw){
        const val = (raw||'').trim();
        if(!val) return;
        frame.src = (val.match(/^https?:\/\//i) ? '' : 'https://') + val;
      }
      if(btn){ btn.addEventListener('click', ()=>{ loadUrl(input.value); }); }
      try{ const initial = (input && input.value) || DEFAULT_EXTERNAL_URL; loadUrl(initial); }catch(e){ }
  }
})();
